...
startMessage = ['Alameda watershed incremental PRMS calibration started at: ' datestr(now)];
disp(startMessage)
...
[yy1, mm1, dd1] = datevec('10/1/1995'); % Simulation start date
[yy2, mm2, dd2] = datevec('9/30/2014'); % Simulation end date


%% Read in data

% Upload calibration data
[calibFlows, ~] = xlsread('.\calibration_data\alameda_data_20170906_streamflows.xlsx'); % Stream flow data
if Analyze_betterRun
    calibCompRun_AH = xlsread('.\calibration_results\calibResults_RandomRunsGroup001_run322_248_350.xlsx', 'run322_AH');
    calibCompRun_SA = xlsread('.\calibration_results\calibResults_RandomRunsGroup001_run322_248_350.xlsx', 'run350_SA');
    calibCompRun_UA = xlsread('.\calibration_results\calibResults_RandomRunsGroup001_run322_248_350.xlsx', 'run248_UA');
    calibCompRun = {calibCompRun_AH, calibCompRun_SA, calibCompRun_UA};
end
idxNA = find(calibFlows == -999);
calibFlows(idxNA) = NaN;
calibDate = calibFlows(:, 1:3);
calibFlows = calibFlows(:,4:end);
flowForCalib = [3,4,6];  % Select streamflow columns for calibration - same ID #s as PRMS data file
flowForCalibNames = ['Arroyo Hondo', 'San Antonio Creek', 'Upper Alameda'];  % Create vector of streamflow site names for calibration
...
Output = {OutputMat, OutputMat, OutputMat};
    ...
    if Analyze_calibResults
        ...
        if exist(FN, 'file') % Make sure file exists
            ...
            if b.bytes > 5000 % Make sure file is big (not incomplete)
                ...
                % Extract year, month, day, and Arroyo Hondo, San Antonio Creek, and Upper Alameda streamflows from statvar file
                statvarDate = a(:, [2,3,4]);
                statvarFlows = a(:, [10,11,13]);
								statvarPrecip = a(:,[36,37,39]);
                ...
                % Set HY 1996 values for Upper Alameda equal to NaN in both calibFlows and statvarFlows
                idx1996 = find(WY == 1996);
                colUA = 3;    % change this if the Upper Alameda column changes
                flows{1}(idx1996,colUA) = NaN;
                flows{2}(idx1996,colUA) = NaN;
                ...
                for t=1:numel(Output)
                    ...
                    numerator = nansum(matWYMS{2}(:,(t+1)) - matWYMS{1}(:,(t+1)));
                    denominator = nansum(matWYMS{1}(:,t+1));
                    ...
                    for i = 1:numel(flows)
                        [flowsSort{i}(:,t), idxSort{i}(:,t)] = sort(flows{i}(:,t), 'descend');
                    end
                    
                    % calculate rank
                    sortRank = 1:numel(flowsSort{i}(:,t));
                    
                    % calculate exceedence probability
                    excProb = flowsSort;
                    for i = 1:numel(flowsSort)
                        for m=1:numel(flowsSort{i}(:,t))
                            
                            excProb{i}(m,t) = 100 * (sortRank(m) / (numel(flowsSort{i}(:,t)) + 1));
                    ...
                    m1=20;
                    m2=70;
                    QSm1 = interp1(excProb{2}(:,t), flowsSort{2}(:,t), m1);
                    QSm2 = interp1(excProb{2}(:,t), flowsSort{2}(:,t), m2);
                    QOm1 = interp1(excProb{1}(:,t), flowsSort{1}(:,t), m1);
                    QOm2 = interp1(excProb{1}(:,t), flowsSort{1}(:,t), m2);
                    tinyNumber = 0.001;
                    ...
                    for i=1:numel(excProb)
                        idxCell{i}{t} = find(excProb{i}(:,t) < 20);
                    end
                    numerator = nansum(flowsSort{2}(idxCell{2}{t},t) - flowsSort{1}(idxCell{1}{t},t));
                    denominator = nansum(flowsSort{1}(idxCell{1}{t},t));
                    ...
                    QSmin = min(flowsSort{2}(:,t));
                    QOmin = min(flowsSort{1}(:,t));
                    idxCell = cell(size(flowsSort));
                    for i=1:numel(excProb)
                        idxCell{i}{t} = find(excProb{i}(:,t) > 70);
                    end
                    tinyNumber = 0.001;
                    numerator1 = nansum(log10(flowsSort{2}(idxCell{2}{t},t) + tinyNumber) - log10(QSmin + tinyNumber));
                    numerator2 = nansum(log10(flowsSort{1}(idxCell{1}{t},t) + tinyNumber) - log10(QOmin + tinyNumber));
                   ...
                   % statvarPrecip
                   excProbUnsortIdx = 1:numel(excProb{1}(:,t));
                   idxOrigObs(idxSort{1}(:,t)) = excProbUnsortIdx;
                   idxOrigSim(idxSort{2}(:,t)) = excProbUnsortIdx;
                   excProbUnsort = cell(size(excProb));
                   excProbUnsort{1}(:,t) = excProb{1}(idxOrigObs,t);
                   excProbUnsort{2}(:,t) = excProb{2}(idxOrigSim,t);
                   ...
                   for i=1:numel(excProbUnsort)
                       idxCell{i}{t} = find(excProbUnsort{i}(:,t) > 20);
                       flowsCut{i}(idxCell{i}{t},t) = NaN;
                   end
                   

                   % calculate cross-correlation
                   isOkObs = isfinite(flowsCut{1}(:,t));
                   isOkSim = isfinite(flowsCut{2}(:,t));
                   [lagObs, ckObs, rObs, tdObs] = xcorrTD(statvarPrecip(isOkObs,t), flowsCut{1}(isOkObs,t));
                   [lagSim, ckSim, rSim, tdSim] = xcorrTD(statvarPrecip(isOkSim,t), flowsCut{2}(isOkSim,t));
                   ...
                   % 'Percent Bias Time Lag'
                    ...
                    numer = log10(nanmedian(flowsSort{2}(:,t))) - log10(nanmedian(flowsSort{1}(:,t)));
                    denom = log10(nanmedian(flowsSort{1}(:,t)));
