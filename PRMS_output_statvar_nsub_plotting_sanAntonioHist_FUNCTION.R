statvar_nsub_plotting <- function(plots_OnOff, subbasin_OnOff, runFolder){
  ...
  gaugedPrecipReadIn<- read.csv("./calibration_data/alameda_data_20170906_precip_SanAntCkHist.csv", header=TRUE, 
                                na.strings="-999", stringsAsFactors=FALSE)
  ...
  if (plots_OnOff[23] == 1){
    ...
    delta_hru_storage[2:6574,8:19] <- diff(as.matrix(allVar_hydYear$hru_storage[,8:19]))
  ...
  if (plots_OnOff[26] == 1){
    ...
    numDateCol <- 7
    subbasinIdxNoDate <- subbasinIdx[(numDateCol + 1): length(subbasinIdx)]
    flowList <- list(data.frame(allVar_hydYear$sub_cfs[,subbasinIdxNoDate]),
                     data.frame(allVar_hydYear$runoff[,subbasinIdxNoDate]))
    ...
    for (i in 1:length(flowList)){
      ...
      for (q in 1:length(years)){
        ...
        flow <- data.frame(flowListHY[[i]][[q]])
        flowSorted <- flow
